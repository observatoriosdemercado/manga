...
#Entrando dados no R
dados1 <- read.csv2('janeiro_2026.csv', header=T, sep=";", dec = ".")
...
saldo_26 <- dados1a |> filter(ano == "2026") |> select(ano, mes, Saldo_Manga) |> mutate(mes = as.character(mes))

saldom <- rbind(saldo_25, saldo_26)   # agora não vira NA
# ou, melhor:
saldom <- bind_rows(saldo_25, saldo_26)
...
saldo_26 <- dados1a |> filter(ano == "2026") |> select(ano, mes, Saldo_Manga)

saldom <- rbind(saldo_25, saldo_26)   # funciona porque os levels são idênticos
...
total_26$mes <- factor(total_26$mes,levels = c("Janeiro", "Fevereiro", "Março", "Abril", "Maio", "Junho", "Julho", "Agosto", "Setembro", "Outubro", "Novembro", "Dezembro"))
...
totalm <- totalm %>%
  rename("Contratados 2025" = Contratados_Manga.x,
         "Demitidos 2025" = Demitidos_Manga.x,
         "Contratados 2026" = Contratados_Manga.y,
         "Demitidos 2026" = Demitidos_Manga.y,
         "Saldo 2025" = Saldo_Manga.x,
         "Saldo 2026" = Saldo_Manga.y)

#Entrando com o restante dos dados

load("janeiro_26_caged.RData")
...
### pela mangicultura em `r strftime(atual, format = "%B")` de 2026 
``` {r emp1}
...
### Saldo Gerado pela mangicultura em `r strftime(atual, format = "%B")` de 2026 
``` {r emp3}
...
g1 <- ggplot() +  #estetica vai valer para todos os geom's
  geom_col(data=saldom, aes(x=mes, y=Saldo_Manga, fill=ano), size=2, width = 0.9, position = "dodge")+
    ...
    labs(y= "Saldo de Emprego (unidade)", x= "Meses do Ano", title='Saldo mensal de Empregos na Mangicultura: 2025 e 2026',
       caption = "")+
...
g4 <- ggplot(data = subclasse_positivos, aes(x = grau_instrucao, y = ..count..)) +
  geom_bar(fill='#1e4971')+
    scale_y_continuous(limits=c(0, 250), n.breaks = 10, expand = expansion(add=c(0,0.5)))+
