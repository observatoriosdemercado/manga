...
library(lubridate)
data <- Sys.Date() #ajustar semanalmente
#last <- today - 7
#sem_ano <- isoweek(data) #ajustar semanalmente

#data <- as.Date('2026-02-13')
sem_ano <- isoweek(data) 
#sem_ano <- 43
...
date <- seq(as.Date('2025-01-01'),to=as.Date('2025-12-01'),by='1 month')

#Entrando dados no R
dados <- read_excel("manga_ibge2024.xlsx", col_names = TRUE)
...
### Área Plantada com Manga no Brasil em 2024 
``` {r}
num <- 86.5 
valueBox(
...
### Produtividade média da Manga no Brasil em 2024 
``` {r}
num <- 21
valueBox(
...
### Volume com Manga produzida no Brasil em 2024 
``` {r}
num <- 1.84
valueBox(
...
### Valor Produção com Manga produzida no Brasil em 2024 
``` {r}
num <- 3.6
valueBox(
...
g1 <- dados %>% 
  ...
  labs(y= "Hectares", x= "Anos", title='Evolução da Área com Manga no Brasil: 2011-2024',
       caption = "Fonte: IBGE (2025)") +
  scale_y_continuous(limits=c(0, 90000), n.breaks = 10, expand = expansion(add=c(0,0.5)))+
...
g2 <- dados %>% 
  ...
  labs(y= "Mil Toneladas", x= "Anos", title='Evolução do Volume com Manga no Brasil: 2011-2024',
       caption = "Fonte: IBGE (2025)") +
  scale_y_continuous(limits=c(0, 2000), n.breaks = 6, expand = expansion(add=c(0,0.5)))+
  scale_x_continuous(breaks = seq(2011, 2024, by = 1))+
  theme_classic()+ #Definindo tema
  theme(axis.text.x=element_text(angle=0, hjust=0.5, size=12, margin = margin(b=5)),
        axis.text.y=element_text(hjust=1, size=12, margin = margin(l=20)),
...
g3 <- dados %>% 
  ...
  labs(y= "Produtividade (Ton/Ha)", x= "Anos", title='Evolução da Produtividade com Manga no Brasil: 2011-2024',
       caption = "IBGE (2025)") +
...
labs(y= "VBP (Milhões R$)", x= "Anos", title='Evolução do Valor Bruto da Produção com Manga no Brasil: 2011-2024',
       caption = "IBGE (2025)") +
  scale_y_continuous(limits=c(0, 3800), n.breaks = 8, expand = expansion(add=c(0,0.5)))+
  scale_x_continuous(breaks = seq(2011, 2024, by = 1))+
  theme_classic()+ #Definindo tema
  theme(axis.text.x=element_text(angle=0, hjust=0.5, size=12, margin = margin(b=5)),
        axis.text.y=element_text(hjust=1, size=12, margin = margin(l=20)),
...
### Área Plantada com Manga no Vale S. Francisco em 2024 
``` {r}
num <- 44.4 
valueBox(
  value = paste0(num, " Mil Hectares"),
...
### Produtividade média da Manga no Vale S. Francisco em 2024 
``` {r}
num <- 28
valueBox(
  value = paste0( num, " Toneladas/Hectare"),
...
### Volume com Manga produzida no Vale S. Francisco em 2024 
``` {r}
num <- 1.24
valueBox(
  value = paste0(format(num, decimal.mark =  ","), " Milhões Toneladas" ),
#  color = "lightblue3",
  icon= "fa-pen")
```
...
g1 <- dados %>% 
  ...
  scale_fill_manual(values=mycolor1)+
  labs(y= "Hectares", x= "Anos", title='Evolução da Area com Manga no Vale do São Francisco: 2011-2024',
       caption = "Fonte: IBGE (2025)") +
  scale_y_continuous(limits=c(0, 45000), n.breaks = 8, expand = expansion(add=c(0,0.5)))+
  scale_x_continuous(breaks = seq(2011, 2024, by = 1))+
  theme_classic()+ #Definindo tema
  theme(axis.text.x=element_text(angle=0, hjust=0.5, size=12, margin = margin(b=5)),
        axis.text.y=element_text(hjust=1, size=12, margin = margin(l=20)),
...
g2 <- dados %>% 
  ...
  geom_col(aes(x=ano, y=quantidade/1000, fill="Volume (Mil Toneladas)"), lwd=1)+
  scale_fill_manual(values=mycolor2)+
  labs(y= "Mil Toneladas", x= "Anos", title='Evolução do Volume com Manga no Vale do São Francisco: 2011-2024',
       caption = "Fonte: IBGE (2025)") +
  scale_y_continuous(limits=c(0, 1300), n.breaks = 6, expand = expansion(add=c(0,0.5)))+
  scale_x_continuous(breaks = seq(2011, 2024, by = 1))+
  theme_classic()+ #Definindo tema
  theme(axis.text.x=element_text(angle=0, hjust=0.5, size=12, margin = margin(b=5)),
        axis.text.y=element_text(hjust=1, size=12, margin = margin(l=20)),
...
g3 <- dados %>% 
  ...
  geom_col(aes(x=ano, y=produtividade, fill="Produtividade (Toneladas/Hectares)"), lwd=1)+
  scale_fill_manual(values=mycolor3)+
  labs(y= "Produtividade (Ton/Ha)", x= "Anos", title='Evolução da Produtividade com Manga no Vale do São Francisco: 2011-2024',
       caption = "IBGE (2025)") +
  scale_y_continuous(limits=c(0, 33), n.breaks = 6, expand = expansion(add=c(0,0.5)))+
  scale_x_continuous(breaks = seq(2011, 2024, by = 1))+
  theme_classic()+ #Definindo tema
  theme(axis.text.x=element_text(angle=0, hjust=0.5, size=12, margin = margin(b=5)),
        axis.text.y=element_text(hjust=1, size=12, margin = margin(l=20)),
...
g4 <- dados %>% 
  ...
  geom_col(aes(x=ano, y=vbp/1000, fill="Valor Bruto Produção (Milhões R$)"), lwd=1)+
  scale_fill_manual(values=mycolor4)+
  labs(y= "VBP (Milhões R$)", x= "Anos", title='Evolução do Valor Bruto da Produção com Manga no Vale do São Francisco: 2011-2024',
       caption = "IBGE (2025)") +
  scale_y_continuous(limits=c(0, 2500), n.breaks = 5, expand = expansion(add=c(0,0.5)))+
  scale_x_continuous(breaks = seq(2011, 2024, by = 1))+
  theme_classic()+ #Definindo tema
  theme(axis.text.x=element_text(angle=0, hjust=0.5, size=12, margin = margin(b=5)),
        axis.text.y=element_text(hjust=1, size=12, margin = margin(l=20)),
...
#Entrando dados no R
dados1 <- read.csv2('dezembro_2025.csv', header=T, sep=";", dec = ".")
...
saldo_24 <- dados1a |> filter(ano == "2024") |> select(ano, mes, Saldo_Manga) |> mutate(mes = as.character(mes))
saldo_25 <- dados1a |> filter(ano == "2025") |> select(ano, mes, Saldo_Manga) |> mutate(mes = as.character(mes))

saldom <- rbind(saldo_24, saldo_25)   # agora não vira NA
# ou, melhor:
saldom <- bind_rows(saldo_24, saldo_25)
...
dados1a <- dados1 |>
  mutate(
    ano = format(date, "%Y"),
    mes = factor(format(date, "%B"), levels = mes_levels, ordered = TRUE)
  )

saldo_24 <- dados1a |> filter(ano == "2024") |> select(ano, mes, Saldo_Manga)
saldo_25 <- dados1a |> filter(ano == "2025") |> select(ano, mes, Saldo_Manga)

saldom <- rbind(saldo_24, saldo_25)   # funciona porque os levels são idênticos
...
load("dezembro_25_caged.RData")

#dados_08_vsf$sexo <- lapply(dados_08_vsf$sexo, factor)

dados_12_vsf$sexo[dados_12_vsf$sexo == 1] <- 0
dados_12_vsf$sexo[dados_12_vsf$sexo == 3] <- 1

dados_12_vsf <- mutate(dados_12_vsf,
                sexo = factor(sexo, levels = 0:1, labels = c("Masculino", "Feminino")))

# Mantendo apenas 

#Separando as admissoes e as demissoes
dados_12_vsf_positivos <-dados_12_vsf%>% filter(saldo_movim=="1")
dados_12_vsf_negativos <-dados_12_vsf%>% filter(saldo_movim=="-1")

#Mantendo apenas o agronegocio
secao_positivos <-dados_12_vsf_positivos%>% filter(secao=="A")
secao_negativos <-dados_12_vsf_negativos%>% filter(secao=="A")

#Mantendo apenas a manga
subclasse_positivos <-dados_12_vsf_positivos%>% filter(subclasse=="133410")
subclasse_negativos <-dados_12_vsf_negativos%>% filter(subclasse=="133410")
...
### pela mangicultura em `r strftime(atual, format = "%B")` de 2025 
``` {r emp1}
num <- tail(dados1$Contratados_Manga,1)
valueBox(
...
### pela mangicultura em `r strftime(atual, format = "%B")` de 2025 
``` {r emp2}
num <- tail(dados1$Demitidos_Manga,1)
valueBox(
...
### Saldo Gerado pela mangicultura em `r strftime(atual, format = "%B")` de 2025 
``` {r emp3}
num <- tail(dados1$Saldo_Manga,1)
valueBox(
...
g1 <- ggplot() +  #estetica vai valer para todos os geom's
  geom_col(data=saldom, aes(x=mes, y=Saldo_Manga, fill=ano), size=2, width = 0.9, position = "dodge")+
    ...
    labs(y= "Saldo de Emprego (unidade)", x= "Meses do Ano", title='Saldo mensal de Empregos na Mangicultura: 2024 e 2025',
       caption = "")+
...
g3 <- ggplot(data=subclasse_positivos, aes(x=idade))+
  geom_histogram(binwidth=3, fill="#69b3a2", color="#e9ecef", alpha=0.9)+
    scale_x_continuous(limits=c(18, 70), n.breaks = 10, expand = expansion(add=c(0,0.5)))+
...
today <- Sys.Date() #ajustar semanalmente
#today <- as.Date('2026-02-13')
atual <-  today
data <-  today

#today <- as.Date('2025-02-07')
#atual <-  today
#data <-  today
...
preco_palmer_2025 <- window(preco_palmer, end=c(2025,52))
seas25 <- seasplot(preco_palmer_2025, trend=F, outplot = F)
...
preco_palmer_26 <- as.matrix(tail(dadosp$preco_def,sem_ano)) 
preco_palmer_2026 <- matrix(NA, nrow=52, ncol=1)

for(i in 1:sem_ano){
  preco_palmer_2026[i,1] = preco_palmer_26[i,1]
}
...
time <- seq(1:52)
table <- data.frame(time, matrix[,1], round(medias25,2), matrix[,2], round(tail(preco_palmer_2024,52),2),
                    round(tail(preco_palmer_2025,52),2), preco_palmer_2026[,1])
colnames(table) = c('Semanas', 'Mínimo', 'Média', 'Máximo', 'Ano 2024', 'Ano 2025', 'Ano 2026')
...
#table_db <- data.frame(matrix[,1], round(medias24,2), matrix[,2], round(tail(preco_palmer_2023,52),2),
 #                   round(tail(preco_palmer_2024,52),2), #preco_palmer_2025[,1])
#colnames(table_db) = c('Mínimo', 'Média', 'Máximo', '2023', '2024', '2025')

#Parte 4
preco_palmer_24 <- dadosp %>% filter(ano=='2024')
preco_palmer_2024 <- as.matrix(preco_palmer_24$preco_def)

preco_palmer_25 <- dadosp %>% filter(ano=='2025')
preco_palmer_2025 <- as.matrix(preco_palmer_25$preco_def)
variacao_25 <- (preco_palmer_2025/lag(preco_palmer_2025, 1) - 1)*100

variacao_26 <- (preco_palmer_2026/lag(preco_palmer_2026, 1) - 1)*100
...
colnames(variacaot) = c('Variação em 2025', 'Variação em 2026')
...
### Preço de Palmer na Semana `r strftime(data, format = "%V")` de 2026 
``` {r}
...
### Preço de Palmer na Semana `r strftime(data, format = "%V")` de 2025
``` {r}
num <- preco_palmer_2025[sem_ano, ]
valueBox(
...
g1 <- ggplot()+
  ...
  scale_fill_manual(values=mycolors)+
    geom_line(data=tablea2, aes(x=Semanas, y=value, colour=variable), linetype = "solid",
            size = 1)+
  scale_colour_manual(values = c("red", "chocolate", "darkgreen")) +
  scale_y_continuous(limits = c(0, 9), n.breaks = 8, labels = number_format(accuracy = 0.01,
                                                       decimal.mark = ","))+
  scale_x_continuous(breaks = seq(1, 52, by = 1))+
  labs(y= "Preço Palmer (R$)", x= "Semanas de cada Ano", title='Mínimo, Máximo, Média e preços de 2023 a 2026 de Manga Palmer ao produtor',
...
g3 <- ggplot(data=dadosp, aes(x=date)) +  #estetica vai valer para todos os geom's
  ...
  labs(y= "Preço Palmer (R$)", x= "Semanas de cada Ano", title='Evolução dos preços ao produtor e Tendência Manga Palmer',
       caption = "Fonte: CEPEA reprocessado pelo Observatório de Mercado de Manga da Embrapa") +
  scale_y_continuous(limits=c(0,9), n.breaks = 9, expand = expansion(add=c(0,0.5)), 
...
#Analise das comparações entre as médias
#Analise das comparações entre as médias
preco_tommy_2025 <- window(preco_tommy, end=c(2025,52))
seas25 <- seasplot(preco_tommy_2025, trend=F, outplot = F)
medias25 <- colMeans(seas25$season)
...
preco_tommy_26 <- as.matrix(tail(dadosp$preco_def,sem_ano)) 
preco_tommy_2026 <- matrix(NA, nrow=52, ncol=1)

for(i in 1:sem_ano){
  preco_tommy_2026[i,1] = preco_tommy_26[i,1]
}
...
time <- seq(1:52)
table <- data.frame(time, matrix[,1], round(medias25,2), matrix[,2], round(tail(preco_tommy_2024,52),2),
                    round(tail(preco_tommy_2025,52),2), preco_tommy_2026[,1])
colnames(table) = c('Semanas', 'Mínimo', 'Média', 'Máximo', 'Ano 2024', 'Ano 2025', 'Ano 2026')
...
#table_db <- data.frame(time, matrix[,1], round(medias24,2), matrix[,2], round(tail(preco_tommy_2023,52),2),
#                    round(tail(preco_tommy_2024,52),2), preco_tommy_2025[,1])
#colnames(table_db) = c('Semanas', 'Mínimo', 'Média', 'Máximo', 'Ano 2023', 'Ano 2024', 
 #                   'Ano 2025')
...
preco_tommy_25 <- dadosp %>% filter(ano=='2025')
preco_tommy_2025 <- as.matrix(preco_tommy_25$preco_def)
variacao_25 <- (preco_tommy_2025/lag(preco_tommy_2025, 1) - 1)*100

variacao_26 <- (preco_tommy_2026/lag(preco_tommy_2026, 1) - 1)*100
...
### Preço de Tommy na Semana `r strftime(data, format = "%V")` de 2026
``` {r}
...
### Preço de Tommy na Semana `r strftime(data, format = "%V")` de 2025
``` {r}
num <- preco_tommy_2025[sem_ano, ]
valueBox(
  value = paste0( "R$", format(round(num,2), decimal.mark =  ",")),
#  color = "#edc40c",
  icon= "fa-pen")
...
g5 <- ggplot()+
    ...
    scale_y_continuous(limits = c(0, 9), n.breaks = 8, labels = number_format(accuracy = 0.01,
                                                                              decimal.mark = ","))+
    scale_x_continuous(breaks = seq(1, 52, by = 1))+
    labs(y= "Preço Tommy (R$)", x= "Semanas de cada Ano", title='Mínimo, Máximo, Média e preços de 2023 a 2025 de Manga Tommy ao produtor',
         caption = "Fonte: CEPEA reprocessado pelo Observatório de Mercado de Manga da Embrapa")+
    theme_minimal()+
    theme(axis.text.x=element_text(angle=0, hjust=0.5, size=8, margin = margin(b=10)),
          axis.text.y=element_text(hjust=0.5, size=8, margin = margin(l=10)),
