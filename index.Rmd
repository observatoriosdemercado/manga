...
library(lubridate)
data <- as.Date("2023-06-23") #ajustar semanalmente
sem_ano <- 25 #ajustar semanalmente
...
#Entrando dados no R
dados1 <- read.csv2('abril_2023.csv', header=T, sep=";", dec = ".")
...
load("abril_23_caged.RData")

#dados_02_vsf$sexo <- lapply(dados_02_vsf$sexo, factor)

dados_04_vsf$sexo[dados_04_vsf$sexo == 1] <- 0
dados_04_vsf$sexo[dados_04_vsf$sexo == 3] <- 1

dados_04_vsf <- mutate(dados_04_vsf,
                sexo = factor(sexo, levels = 0:1, labels = c("Masculino", "Feminino")))

# Mantendo apenas 

#Separando as admissoes e as demissoes
dados_04_vsf_positivos <-dados_04_vsf%>% filter(saldo_movim=="1")
dados_04_vsf_negativos <-dados_04_vsf%>% filter(saldo_movim=="-1")

#Mantendo apenas o agronegocio
secao_positivos <-dados_04_vsf_positivos%>% filter(secao=="A")
secao_negativos <-dados_04_vsf_negativos%>% filter(secao=="A")

#Mantendo apenas a manga
subclasse_positivos <-dados_04_vsf_positivos%>% filter(subclasse=="133410")
subclasse_negativos <-dados_04_vsf_negativos%>% filter(subclasse=="133410")
...
### pela mangicultura em `r strftime(atual, format = "%B")` de 2023 
``` {r emp1}
num <- 829
valueBox(
...
### Saldo Gerado pela mangicultura em `r strftime(atual, format = "%B")` de 2023 
``` {r emp3}
num <- 248
valueBox(
...
g1 <- ggplot() +  #estetica vai valer para todos os geom's
  geom_col(data=saldom, aes(x=mes, y=Saldo_Manga, fill=ano), size=2, width = 0.9, position = "dodge")+
    ...
    labs(y= "Saldo de Emprego (unidade)", x= "Meses do Ano", title='Saldo mensal de Empregos na Mangicultura: 2022 e 2023',
       caption = "")+
...
g1 <- ggplot()+
  ...
  labs(y= "Preço R$", x= "Semanas de cada Ano", title='Mínimo, Máximo, Média e preços de 2020 a 2023 de Manga Palmer ao produtor',
...
g5 <- ggplot()+
    ...
    scale_y_continuous(limits = c(0, 8), n.breaks = 8, labels = number_format(accuracy = 0.01,
                                                                              decimal.mark = ","))+
    scale_x_continuous(breaks = seq(1, 52, by = 1))+
    labs(y= "Preço R$", x= "Semanas de cada Ano", title='Mínimo, Máximo, Média e preços de 2020 a 2023 de Manga Tommy ao produtor',
         caption = "Fonte: CEPEA reprocessado pelo Observatório de Mercado de Manga da Embrapa")+
    theme_minimal()+
    theme(axis.text.x=element_text(angle=0, hjust=0.5, size=8, margin = margin(b=10)),
          axis.text.y=element_text(hjust=0.5, size=8, margin = margin(l=10)),
