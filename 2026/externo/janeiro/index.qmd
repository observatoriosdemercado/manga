...
library(lubridate)
anterior <- as.Date("2025-12-01")
atual <-  as.Date("2026-01-01") #ultimo mes disponibilizado
today <- as.Date("2026-01-30") #data para o IGPI - ultima semana do preço
mes <- 1
```
...
Em relação aos volumes, em `r strftime(atual, format = "%B")` foram exportadas cerca de 12,9 mil toneladas (t) de mangas pelo Brasil. Este valor é maior do que a média histórica (7 mil t), mais alto do que os volumes de 2024 (11 mil t) e o volume de 2025 (11,1 mil t). 

Em termos dos valores das exportações, em `r strftime(atual, format = "%B")` foram, aproximadamente, US\$ 15,6 milhões. Este valor supera a média histórica (US\$ 7,6 milhões), o valor de 2025 (US\$ 9,5 milhões), mas é menor do que o valor de 2024 US\$ 16,4 milhões.

As exportações se concentraram, principalmente, nas variedades Palmer e Tommy para a Europa. Em relação aos principais destinos, no ano, 42,5% das mangas foram enviadas para a Holanda, 28,3% para a Espanha e 11,39% para o Reino Unido. 

A principal via de envio foi a marítima (86,45%), com 78,5% do total despachado pela alfândega de Fortaleza e 7,2% pelo Aeroporto de Guarulhos-SP.

A maior parte das frutas teve origem em Pernambuco (64,39%) e na Bahia (26,6%). Os estados de São Paulo (6,4%) e Rio Grande do Norte (1,44%), também contribuíram para os resultados obtidos com as exportações de manga.

A taxa de câmbio esperada pelo mercado para o final do ano de 2026 é de R$5,50 para 1 dólar.
...
dados21$URF <- factor(dados21$URF, levels = c("ALF-Fortaleza", "Aeroporto de GRU","IRF Natal", "Aeroporto do Rio de Janeiro", "Aeroporto de VCP", "ALF-Salvador", "ALF-Dionísio Cerqueira", "ALF-Belo Horizonte", "ALF-Foz do Iguaçu",  "Outros"))
