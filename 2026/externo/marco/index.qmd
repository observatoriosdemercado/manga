...
library(lubridate)
anterior <- as.Date("2026-02-01")
atual <-  as.Date("2026-03-01") #ultimo mes disponibilizado
today <- as.Date("2026-04-03") #data para o IGPI - ultima semana do preço
mes <- 3
```
...
Em relação aos volumes, em `r strftime(atual, format = "%B")` foram exportadas cerca de 24,8 mil toneladas (t) de mangas pelo Brasil. Este valor é maior do que a média histórica (11,6 mil t), mais alto do que os volumes de 2024 (21 mil t) e o volume de 2025 (18 mil t). No ano, foram exportadas 55,5 mil t, um aumento de 40% em relação à 2025.

Em termos dos valores das exportações, em `r strftime(atual, format = "%B")` foram, aproximadamente, US\$ 29,6 milhões. Este valor supera a média histórica (US\$ 13,7 milhões), o valor de 2025 (US\$ 19,6 milhões), mas é menor do que o valor de 2024 US\$ 36,1 milhões.

As exportações se concentraram, principalmente, nas variedades Palmer e Tommy para a Europa, mas também houve exportação para os Estados Unidos. Em relação aos principais destinos, no ano, 48,96% das mangas foram enviadas para a Holanda, 25,35% para a Espanha e 8,96% para o Reino Unido. 

A principal via de envio foi a marítima (87,83%), com 79,2% do total despachado pela alfândega de Fortaleza-CE e 6,15% pelo Aeroporto de Guarulhos-SP.

A maior parte das frutas teve origem em Pernambuco (63,30%) e na Bahia (25,83%). Os estados de São Paulo (4,66%) e Rio Grande do Norte (2,96%), também contribuíram para os resultados obtidos com as exportações de manga.

A taxa de câmbio esperada pelo mercado para o final do ano de 2026 é de R$5,4 para 1 dólar.
...
dados21$URF <- factor(dados21$URF, levels = c("ALF-Fortaleza", "Aeroporto de GRU", "IRF Natal", "ALF-Foz do Iguaçu",  "ALF-Dionísio Cerqueira", "Aeroporto de VCP", "ALF-Salvador", "Aeroporto do Rio de Janeiro", "Aeroporto de Salvador",  "Outros"))
...
# 9. Evolução da Taxa de Câmbio Esperada para Dezembro de 2026
...
<center>![](apoio4.png){width="7cm"}</center>
