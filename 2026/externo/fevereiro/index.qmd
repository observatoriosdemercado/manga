...
atual <-  as.Date("2026-02-01") #ultimo mes disponibilizado
today <- as.Date("2026-02-27") #data para o IGPI - ultima semana do preço
mes <- 2
```
...
Em relação aos volumes, em `r strftime(atual, format = "%B")` foram exportadas cerca de 17,8 mil toneladas (t) de mangas pelo Brasil. Este valor é maior do que a média histórica (7,3 mil t), mais alto do que os volumes de 2024 (13,5 mil t) e o volume de 2025 (10,4 mil t). No ano, foram exportadas 30,7 mil t, um aumento de 42,4% em relação à 2025.

Em termos dos valores das exportações, em `r strftime(atual, format = "%B")` foram, aproximadamente, US\$ 20,8 milhões. Este valor supera a média histórica (US\$ 8,2 milhões), o valor de 2025 (US\$ 9,9 milhões), mas é menor do que o valor de 2024 US\$ 22,9 milhões.

As exportações se concentraram, principalmente, nas variedades Palmer e Tommy para a Europa, mas também houve exportação para os Estados Unidos. Em relação aos principais destinos, no ano, 46,3% das mangas foram enviadas para a Holanda, 28,3% para a Espanha e 8,07% para o Reino Unido. 

A principal via de envio foi a marítima (87,25%), com 79,2% do total despachado pela alfândega de Fortaleza-CE e 7,1% pelo IRF de Natal-RN.

A maior parte das frutas teve origem em Pernambuco (65,68%) e na Bahia (23%). Os estados de São Paulo (5,8%) e Rio Grande do Norte (2,37%), também contribuíram para os resultados obtidos com as exportações de manga.

A taxa de câmbio esperada pelo mercado para o final do ano de 2026 é de R$5,43 para 1 dólar.
...
dados21$URF <- factor(dados21$URF, levels = c("ALF-Fortaleza", "IRF Natal", "Aeroporto de GRU", "Aeroporto de VCP", "Aeroporto do Rio de Janeiro",  "ALF-Foz do Iguaçu", "ALF-Salvador", "ALF-Dionísio Cerqueira", "IRF Porto Xavier",  "Outros"))
