...
O preço médio de **Palmer** na semana `r strftime(today, format = "%V de %Y")` foi R\$ 1,94, variando entre R\$ 2,30 e R\$ 1,70. A média histórica dos dados deflacionados, desde 2012 até 2025, para esta semana, é R\$ 3,27.  Em 2024, o preço deflacionado é de R\$ 2,40 para a semana `r strftime(today, format = "%V")` e, em 2025, o preço é R\$ 2,62. Em relação à semana `r strftime(last, format = "%V")`, os preços aumentaram 1,04%. 

No caso da **Tommy**, na semana `r strftime(today, format = "%V de %Y")`, os preços médios foram de R\$ 2,51, variando entre R\$ 2,80 e R\$ 2,30. Este valor é menor do que a média histórica para esta semana (R\$ 2,60), é menor do que o preço em 2025 (R\$ 2,69), mas é maior do que o valor de 2024 (R\$ 1,11). Em relação à semana anterior, os preços reduziram -10,04%. 

Sobre os preços de exportação, a **Palmer exportação** foi vendida na média por R\$ 2,46, variando entre R\$3,00 e R\$2,20, aumento de 4% em relação à semana anterior. A **Tommy Exportação para a Europa** teve média de R\$3,00, variando entre R\$3,20 e R\$2,80, estável na comparação com a última semana. No momento, os preços das mangas palmer e tommy brasileiras na Europa estão mais baixos devido a existência de mangas Kent do Peru e também a chegada das Kent da Costa do Marfim. 
...
colnames(table) = c('Semanas', 'Mínimo', 'Média', 'Máximo', 'Ano 2024', 'Ano 2025', 
                    'Ano 2026')
tablea <- table[,-c(5:7)]
...
preco_palmer_2023_5 <- window(preco_palmer, start=c(2023,1), end=c(2025,52))
seas23_5<-seasplot(preco_palmer_2023_5, trend=F, outplot = F)
medias23_5 <- colMeans(seas23_5$season)
...
#time <- seq(1:52)
table <- data.frame(time, round(medias23_5,2), round(tail(preco_palmer_2024,52),2),
                    round(tail(preco_palmer_2025,52),2), preco_palmer_2026[,1])
colnames(table) = c('Semanas', 'Média 23_25', 'Ano 2024', 'Ano 2025', 
                    'Ano 2026')
tablea <- table[,-c(3:5)]
tableb <- table[,-c(2)]
...
preco_tommy_2023_5 <- window(preco_tommy, start=c(2023,1), end=c(2025,52))
seas23_5<-seasplot(preco_tommy_2023_5, trend=F, outplot = F)
medias23_5 <- colMeans(seas23_5$season)

#Como só se tem até a semana 52
medias23_5 <- medias23_5[1:52]

table <- data.frame(time, round(medias23_5,2), round(tail(preco_tommy_2024,52),2),
                    round(tail(preco_tommy_2025,52),2), preco_tommy_2026[,1])
colnames(table) = c('Semanas', 'Média 23_25', 'Ano 2024', 'Ano 2025', 
                    'Ano 2026')
tablea <- table[,-c(3:5)]
...
colnames(variacaott) = c('Variação em 2025', 'Variação em 2026')
