...
atual <-  as.Date("2024-06-01") #ultimo mes disponibilizado
today <- as.Date("2024-07-05") #data para o IGPI - ultima semana do preço
mes <- 6
```
...
Em relação aos volumes, em `r strftime(atual, format = "%B")` foram exportadas cerca de 6,5 mil toneladas (t) de mangas pelo Brasil. Este valor é menor do que a média histórica (8,52 mil t), menor do que o volume exportado em 2022 (12 mil t) e inferior ao volume de 2023 (17 mil t). Os valores representam uma redução de -46,23% em relação à 2022 e de -61,82% em relação à 2023.

Em termos dos valores das exportações, em `r strftime(atual, format = "%B")` foram, aproximadamente, US\$ 10 milhões. Em `r strftime(atual, format = "%B")` de 2022, foram gerados cerca de US\$ 11,6 milhões em receitas e, em `r strftime(atual, format = "%B")` de 2023, um total de US\$ 16,7 milhões. Apesar dos valores menores do que nos dois últimos anos, as receitas obtidas estão acima da média histórica.

As exportações se concentraram principalmente na variedade Palmer e Tommy para a Europa. Em relação aos destinos, no ano, 51,13% das mangas foram enviadas para a Holanda, 25,03% para a Espanha, 7,83% para o Portugal, 7,67% para Reino Unido e 2,10% para o Chile. Sobre as vias de exportação, 86,21% foi marítima, 10,77% foi aérea. A maior parte das frutas teve origem em Pernambuco (51,05%) e na Bahia (36,86%). O estado de São Paulo (4,99%) e o Rio Grande do Norte (4,38%) também contribuíram para os resultados obtidos com as exportações de manga.

A taxa de câmbio esperada pelo mercado para o final do ano de 2024 é de R$5,22 para 1 dólar.
