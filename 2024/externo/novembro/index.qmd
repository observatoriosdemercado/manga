...
atual <-  as.Date("2024-11-01") #ultimo mes disponibilizado
...
Em relação aos volumes, em `r strftime(atual, format = "%B")` foram exportadas cerca de 38,7 mil toneladas (t) de mangas pelo Brasil. Este valor é maior do que a média histórica (32,8 mil t), mais alto do que os volumes de 2022, mas inferior ao ano passado. Os valores representam um aumento de 23,21% em relação à 2022 e uma queda de -24,82% em relação à 2023.

Em termos dos valores das exportações, em `r strftime(atual, format = "%B")` foram, aproximadamente, US\$ 43,6 milhões. Em `r strftime(atual, format = "%B")` de 2022, foram gerados cerca de US\$ 27,2 milhões em receitas e, em `r strftime(atual, format = "%B")` de 2023, um total de US\$ 70,9 milhões, justamente o máximo histórico de receita de exportações.

As exportações se concentraram principalmente nas variedades Palmer e Kent para a Europa e Tommy tanto para a Europa quanto Estados Unidos. Em relação aos principais destinos, no ano, 47,39% das mangas foram enviadas para a Holanda, 16,58% para a Espanha e 14,20% para os Estados Unidos. 

A principal via de envio foi a marítima (88,7%), com 81,33% do total despachado pela alfândega de Fortaleza e 5,2% pelo Aeroporto de Guarulhos-SP.

A maior parte das frutas teve origem em Pernambuco (48,13%) e na Bahia (43,73%). O estado de São Paulo (3,71%) e o Rio Grande do Norte (2,22%) também contribuíram para os resultados obtidos com as exportações de manga.

A taxa de câmbio esperada pelo mercado para o final do ano de 2024 é de R$5,69 para 1 dólar.
...
g1 <- ggplot(data=dados1) +  #estetica vai valer para todos os geom's
  ...
  labs(y= "US$ Milhões / Mil Ton", x= "Anos", title='',
       caption = "") +
  scale_y_continuous(limits=c(0, 330), n.breaks = 10, expand = expansion(add=c(0,0.5)))+
