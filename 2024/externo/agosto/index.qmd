...
atual <-  as.Date("2024-08-01") #ultimo mes disponibilizado
today <- as.Date("2024-09-05") #data para o IGPI - ultima semana do preço
mes <- 8
```
...
Em relação aos volumes, em `r strftime(atual, format = "%B")` foram exportadas cerca de 19,9 mil toneladas (t) de mangas pelo Brasil. Este valor é maior do que a média histórica (14,5 mil t), maior do que o volume exportado em 2022 (13 mil t), mas  inferior ao volume de 2023 (20,6 mil t). Os valores representam um aumento de 52,54% em relação à 2022 e queda de -3,31% em relação à 2023.

Em termos dos valores das exportações, em `r strftime(atual, format = "%B")` foram, aproximadamente, US\$ 30,3 milhões. Em `r strftime(atual, format = "%B")` de 2022, foram gerados cerca de US\$ 13 milhões em receitas e, em `r strftime(atual, format = "%B")` de 2023, um total de US\$ 21,5 milhões. A média histórica de receita de exportações é US$ 15 milhões.

As exportações se concentraram principalmente nas variedades Palmer e Tommy para a Europa. Em relação aos destinos, no ano, 51,13% das mangas foram enviadas para a Holanda, 24,51% para a Espanha e 7,14% para o Portugal. 

A principal via de envio foi a maritima (85,70%), com 83,9% do total despachado pela alfândega de Fortaleza e 5,96% pelo Aeroporto de Guarulhos-SP.

A maior parte das frutas teve origem em Pernambuco (53,38%) e na Bahia (36,53%). O estado de São Paulo (4,10%) e o Rio Grande do Norte (3,26%) também contribuíram para os resultados obtidos com as exportações de manga.

A taxa de câmbio esperada pelo mercado para o final do ano de 2024 é de R$5,35 para 1 dólar.
