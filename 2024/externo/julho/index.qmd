...
atual <-  as.Date("2024-07-01") #ultimo mes disponibilizado
today <- as.Date("2024-08-02") #data para o IGPI - ultima semana do preço
mes <- 7
```
...
Em relação aos volumes, em `r strftime(atual, format = "%B")` foram exportadas cerca de 11,2 mil toneladas (t) de mangas pelo Brasil. Este valor é maior do que a média histórica (8,9 mil t), mas é menor do que o volume exportado em 2022 (11,9 mil t) e inferior ao volume de 2023 (14,2 mil t). Os valores representam uma redução de -5,89% em relação à 2022 e de -20,81% em relação à 2023.

Em termos dos valores das exportações, em `r strftime(atual, format = "%B")` foram, aproximadamente, US\$ 17 milhões. Em `r strftime(atual, format = "%B")` de 2022, foram gerados cerca de US\$ 11,9 milhões em receitas e, em `r strftime(atual, format = "%B")` de 2023, um total de US\$ 14,5 milhões. A média histórica de receita de exportações é uS$9,73 milhões.

As exportações se concentraram principalmente na variedade Palmer e Tommy para a Europa. Em relação aos destinos, no ano, 50,48% das mangas foram enviadas para a Holanda, 25,48% para a Espanha, 8,02% para o Portugal, 7,39% para Reino Unido e 2,38% para o Chile. Sobre as vias de exportação, 85,37% foi marítima, 11,09% foi aérea. A maior parte das frutas teve origem em Pernambuco (52,47%) e na Bahia (36,19%). O estado de São Paulo (4,63%) e o Rio Grande do Norte (3,84%) também contribuíram para os resultados obtidos com as exportações de manga.

A taxa de câmbio esperada pelo mercado para o final do ano de 2024 é de R$5,33 para 1 dólar.
