...
atual <-  as.Date("2024-01-01") #ultimo mes disponibilizado
today <- as.Date("2024-02-09") #data para o IGPI - ultima semana do preço
mes <- 1
```
...
# 5. Principais destinos de exportação da Manga do Brasil:  `r strftime(atual, format = "%B de %Y")`.
...
# 6. Principais vias de exportação da Manga do Brasil: `r strftime(atual, format = "%B de %Y")`. 
...
# 7. Unidade da Federação da Manga exportada do Brasil: `r strftime(atual, format = "%B de %Y")`.
