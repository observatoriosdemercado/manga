...
Em relação aos volumes, em `r strftime(atual, format = "%B")` foram exportadas cerca de 11 mil toneladas (t) de mangas pelo Brasil. Este valor é maior do que o exportado pelo país, no mesmo mês, em 2022 (6,8 mil t), do que em `r strftime(atual, format = "%B")` de 2023 (8,5 mil t), e também supera a média histórica (2012 a 2023), que é de 6,4 mil t, aproximadamente. Os valores representam um crescimento de 60% em relação à 2022 e de 28,4% em relação à 2023.

Em termos dos valores das exportações, em `r strftime(atual, format = "%B")` foram US\$ 14,3 milhões. Em `r strftime(atual, format = "%B")` de 2022, foram gerados cerca de US\$ 6,2 milhões em receitas e, em `r strftime(atual, format = "%B")` de 2023, um total de US\$ 7,3 milhões. 

As exportações se concentraram principalmente na variedade Palmer para a Europa e Tommy para os Estados Unidos e Europa. Em relação aos destinos, em `r strftime(atual, format = "%B")` de 2024, 49,24\% das mangas foram enviadas para a Holanda, 23,23\% para a Espanha, 8,41\% para Portugal, 7,64% para os Estados Unidos e  6,83\% para o Reino Unido. Sobre as vias de exportação, 89\% foi marítima e 11\% foi aérea. A maior parte das frutas teve origem em Pernambuco (45,83\%) e na Bahia (32,65\%). O Rio Grande do Norte (10,71\%), São Paulo (8,47\%) e o Ceará (1,28\%) também contribuíram para os resultados obtidos com as exportações de manga.
