...
library(lubridate)
anterior <- as.Date("2024-01-01")
atual <-  as.Date("2024-02-01") #ultimo mes disponibilizado
today <- as.Date("2024-03-01") #data para o IGPI - ultima semana do preço
mes <- 2
```
...
Em relação aos volumes, em `r strftime(atual, format = "%B")` foram exportadas cerca de 13,5 mil toneladas (t) de mangas pelo Brasil. Este valor é um recorde histórico para um mês de fevereiro, desde quanto as exportações de manga se iniciaram no Brasil. Os valores representam um crescimento de 46,88% em relação à 2022 e de 63,31% em relação à 2023.

Em termos dos valores das exportações, em `r strftime(atual, format = "%B")` foram, aproximadamente, US\$ 21,2 milhões. Em `r strftime(atual, format = "%B")` de 2022, foram gerados cerca de US\$ 8,5 milhões em receitas e, em `r strftime(atual, format = "%B")` de 2023, um total de US\$ 7,7 milhões. Assim, em fevereiro de 2024 se tem também um recorde histórico mensal de receitas de exportação.

As exportações se concentraram principalmente na variedade Palmer e Tommy para a Europa. Em relação aos destinos, no ano 50,71% das mangas foram enviadas para a Holanda, 24,73% para a Espanha, 8,45% para Portugal, 7,05% para o Reino Unido e 4,46% para os Estados Unidos. Sobre as vias de exportação, 89% foi marítima e 11% foi aérea. A maior parte das frutas teve origem em Pernambuco (49,57%) e na Bahia (34,09%). O estado de São Paulo (7,11%) e o Rio Grande do Norte (7%) também contribuíram para os resultados obtidos com as exportações de manga.
...
g10 <- ggplot(data=dados3) +  #estetica vai valer para todos os geom's
  ...
  labs(y= "% do Volume Total Exportados", x= "Países", title='',
       caption = "")+
  scale_y_continuous(limits=c(0, 52), n.breaks = 10, expand = expansion(add=c(0,0.5)))+
   theme_classic()+ #Definindo tema
