...
today <- as.Date("2024-10-04") #data para o IGPI - ultima semana do preço
mes <- 9
```
...
Em relação aos volumes, em `r strftime(atual, format = "%B")` foram exportadas cerca de 29,7 mil toneladas (t) de mangas pelo Brasil. Este valor é maior do que a média histórica (27,5 mil t), mas inferior do que o volume exportado em 2022 (32,2 mil t) e o volume de 2023 (35,5 mil t). Os valores representam uma queda de -7,88% em relação à 2022 e queda de -16,41% em relação à 2023.

Em termos dos valores das exportações, em `r strftime(atual, format = "%B")` foram, aproximadamente, US\$ 38,4 milhões. Em `r strftime(atual, format = "%B")` de 2022, foram gerados cerca de US\$ 27,6 milhões em receitas e, em `r strftime(atual, format = "%B")` de 2023, um total de US\$ 41,9 milhões. A média histórica de receita de exportações é US$ 28,4 milhões.

As exportações se concentraram principalmente nas variedades Palmere Kent para a Europa e Tommy tanto para a Europa quanto Estados Unidos. Em relação aos destinos, no ano, 48,57% das mangas foram enviadas para a Holanda, 20,78% para a Espanha e 9,06% para os Estados Unidos. 

A principal via de envio foi a marítima (86,86%), com 83,1% do total despachado pela alfândega de Fortaleza e 5,6% pelo Aeroporto de Guarulhos-SP.

A maior parte das frutas teve origem em Pernambuco (51,28%) e na Bahia (40,05%). O estado de São Paulo (3,72%) e o Rio Grande do Norte (2,58%) também contribuíram para os resultados obtidos com as exportações de manga.

A taxa de câmbio esperada pelo mercado para o final do ano de 2024 é de R$5,39 para 1 dólar.
...
g11 <- ggplot(data=dados4) +  #estetica vai valer para todos os geom's
  ...
  labs(y= "% de Exportação", x= "Vias", title='',
       caption = "")+
  scale_y_continuous(limits=c(0, 88), n.breaks = 10, expand = expansion(add=c(0,0.5)))+
   theme_classic()+ #Definindo tema
...
dados21$URF <- factor(dados21$URF, levels = c("ALF-Fortaleza", "Aeroporto de GRU", "ALF-Foz", "Aeroporto de VCP", "ALF-Salvador", "Outros"))
