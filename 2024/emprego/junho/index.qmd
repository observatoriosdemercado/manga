...
library(lubridate)
anterior <- as.Date("2024-05-01")
atual <-  as.Date("2024-06-01")
```
...
O saldo de empregos na mangicultura em `r strftime(atual, format = "%B de %Y")` no Vale do São Francisco foi positivo, de 167 pessoas, ou seja, o total de contratações (729) foi maior do que o total de demissões (562). Em relação ao mesmo mês do ano passado, se tem uma piora no saldo total (209 em 2023 e 167 em 2024), com menos pessoas conseguindo emprego. 

Dos contratados no mês, 501 são do sexo masculino (68,72%) e 228 do sexo feminino (31,28%). Em relação à idade, 51,58% se concentra abaixo dos 30 anos: 111 pessoas até 21 anos, 106 até 24 anos, 89 ate 27 anos e 70 até 30 anos. 

Sobre o nível de escolaridade, a maioria das admissões é de pessoas com ensino médio completo (320). Do total de contratados, 43,9% possuem este nível de escolaridade. 

Com relação aos salários pagos, 82,12% dos contratados foram pelo valor de R\$ 1.450,00, mais alto que o salário mínimo nacional, valor acordado pela convenção trabalhista. O maior salário foi R\$ 5.416,00. 
...
#Entrando dados no R
dados1 <- read.csv2('junho_2024.csv', header=T, sep=";", dec = ".")
