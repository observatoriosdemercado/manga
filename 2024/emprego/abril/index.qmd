...
library(lubridate)
anterior <- as.Date("2024-03-01")
atual <-  as.Date("2024-04-01")
```
...
O saldo de empregos na mangicultura em `r strftime(atual, format = "%B de %Y")` no Vale do São Francisco foi positivo, de 146 pessoas, ou seja, o total de contratações (880) foi maior do que o total de demissões (734). Em relação ao mesmo mês do ano passado, se tem uma piora no saldo total (248 em 2023 e 146 em 2024), com menos pessoas conseguindo emprego. 

Dos contratados no mês, 572 são do sexo masculino (71,50%) e 308 do sexo feminino (28,50%). Em relação à idade, 52,95% se concentra abaixo dos 30 anos: 138 pessoas até 21 anos, 130 até 24 anos, 111 ate 27 anos e 87 até 30 anos. 

Sobre o nível de escolaridade, a maioria das admissões é de pessoas com ensino médio completo (447). Do total de contratados, 50,8% possuem este nível de escolaridade. 

Com relação aos salários pagos, 83,41% dos contratados foram pelo valor de R\$ 1.450,00, mais alto que o salário mínimo nacional, valor acordado pela convenção trabalhista. O maior salário foi R\$ 9.500,00. 
...
#Entrando dados no R
dados1 <- read.csv2('abril_2024.csv', header=T, sep=";", dec = ".")
