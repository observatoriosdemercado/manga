...
today <- as.Date("2023-08-04") #data para o IGPI - ultima semana do preço
mes <- 7
...
Em relação aos volumes, em `r strftime(atual, format = "%B")` foram exportadas cerca de 14,2 mil toneladas (t) de mangas pelo Brasil. Este valor é maior do que o exportado pelo país, no mesmo mês, em 2021 (11,1 mil t), e do que em `r strftime(atual, format = "%B")` de 2022 (11,9 mil t), além de superar a média histórica, que é de 7,75 mil t, aproximadamente.

Em termos dos valores das exportações, em `r strftime(atual, format = "%B")` foram US\$ 14,1 milhões. A média histórica (até 2020) para este mês é de US\$ 8,99 milhões. Em `r strftime(atual, format = "%B")` de 2021 foram gerados cerca de US\$ 9,47 milhões em receitas e, em `r strftime(atual, format = "%B")` de 2022, um total de US$ 11,89 milhões.

As exportações se concentraram principalmente na variedade Palmer e Tommy para a Europa. Em relação aos destinos, em 2023, 48,35\% das mangas foram enviadas para a Holanda, 31,96\% para a Espanha, 7,49\% para Portugal e 4,97\% para o Reino Unido. Sobre as vias de exportação, 85,84\% foi marítima, 9,50\% aérea e 4,65\% rodoviária. A maior parte das frutas teve origem em Pernambuco (54,71\%) e na Bahia (37,28\%). São Paulo (3,65\%), Rio Grande do Norte (2,56\%) e Ceará (1,05\%) também contribuíram para os resultados obtidos com as exportações de manga.

A taxa de câmbio esperada pelo mercado para o final do ano é de R\$4,91 para 1 dólar.
