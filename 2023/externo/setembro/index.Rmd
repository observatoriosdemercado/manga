...
library(lubridate)
anterior <- as.Date("2023-08-01")
atual <-  as.Date("2023-09-01") #ultimo mes disponibilizado
today <- as.Date("2023-10-06") #data para o IGPI - ultima semana do preço
mes <- 9
...
Em relação aos volumes, em `r strftime(atual, format = "%B")` foram exportadas cerca de 35,5 mil toneladas (t) de mangas pelo Brasil. Este valor é menor do que o exportado pelo país, no mesmo mês, em 2021 (42,1 mil t), mas é maior do que em `r strftime(atual, format = "%B")` de 2022 (32,2 mil t), além de superar a média histórica, que é de 24,4 mil t, aproximadamente. Nos nove meses do ano, foram exportadas cerca de 141 mil t, um aumento de 14,34% em relação ao mesmo período de 2022.

Em termos dos valores das exportações, em `r strftime(atual, format = "%B")` foram US\$ 36,2 milhões. A média histórica (até 2020) para este mês é de US\$ 26 milhões. Em `r strftime(atual, format = "%B")` de 2021, foram gerados cerca de US\$ 37,5 milhões em receitas e, em `r strftime(atual, format = "%B")` de 2022, um total de US$ 27,6 milhões. Assim, as receitas médias aumentaram, indicando uma valorização da manga brasileira.

As exportações se concentraram principalmente na variedade Palmer, iniciando os embarques de Tommy para os Estados Unidos. Em relação aos destinos, em 2023, 46,32\% das mangas foram enviadas para a Holanda, 23,97\% para a Espanha, 10,62\% para os Estados Unidos, 5,44\% para o Reino Unido e 5,34\% para Portugal. Sobre as vias de exportação, 87,76\% foi marítima, 8,02\% aérea e 4,22\% rodoviária. A maior parte das frutas teve origem em Pernambuco (49,84\%) e na Bahia (43,78\%). São Paulo (2,99\%), Rio Grande do Norte (1,94\%) e Ceará (0,86\%) também contribuíram para os resultados obtidos com as exportações de manga.

A taxa de câmbio esperada pelo mercado para o final do ano é de R\$4,94 para 1 dólar.
