...
library(lubridate)
anterior <- as.Date("2023-10-01")
atual <-  as.Date("2023-11-01") #ultimo mes disponibilizado
today <- as.Date("2023-12-08") #data para o IGPI - ultima semana do preço
mes <- 11
...
Em relação aos volumes, em `r strftime(atual, format = "%B")` foram exportadas cerca de 51,5 mil toneladas (t) de mangas pelo Brasil. Este valor é maior do que o exportado pelo país, no mesmo mês, em 2021 (47,5 mil t), do que em `r strftime(atual, format = "%B")` de 2022 (31,4 mil t), além de superar a máxima histórica (2012 a 2020), que é de 46,9 mil t, aproximadamente. Nos onze meses do ano, foram exportadas cerca de 239 mil t, um aumento de 25,87% em relação ao mesmo período de 2022.

Em termos dos valores das exportações, em `r strftime(atual, format = "%B")` foram US\$ 69,7 milhões. O máximo histórico (até 2020) para este mês é de US\$ 47,9 milhões. Em `r strftime(atual, format = "%B")` de 2021, foram gerados cerca de US\$ 40,1 milhões em receitas e, em `r strftime(atual, format = "%B")` de 2022, um total de US$ 27,2 milhões. Assim, as receitas médias aumentaram, indicando uma valorização da manga brasileira.

As exportações se concentraram principalmente na variedade Palmer para a Europa e Tommy para os Estados Unidos e Europa. Em relação aos destinos, em 2023, 44,02\% das mangas foram enviadas para a Holanda, 18,71\% para a Espanha, 18,70\% para os Estados Unidos, 5,77\% para o Reino Unido e 4\% para Portugal. Sobre as vias de exportação, 89,95\% foi marítima, 6,83\% aérea e 3,22\% rodoviária. A maior parte das frutas teve origem na Bahia (47,91\%) e em Pernambuco (45,29\%). São Paulo (3,07\%), Rio Grande do Norte (2,33\%) e Ceará (0,79\%) também contribuíram para os resultados obtidos com as exportações de manga.

A taxa de câmbio esperada pelo mercado para o final do ano é de R\$4,96 para 1 dólar.
...
<center>![](apoio1.png){width=3cm}</center>
<br>
