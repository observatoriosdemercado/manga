...
today <- as.Date("2023-07-14") #data para o IGPI - ultima semana do preço
mes <- 6
...
Em relação aos volumes, em `r strftime(atual, format = "%B")` foram exportadas 17 mil toneladas (t) de mangas pelo Brasil. Este valor é praticamente o mesmo que foi exportado pelo país, no mesmo mês, em 2021 (17,2 mil t), mas é maior do que em `r strftime(atual, format = "%B")` de 2022 (12 mil t) e supera a média histórica, que é de 6,2 mil t, aproximadamente.

Em termos dos valores das exportações, em `r strftime(atual, format = "%B")` foram US\$ 16,4 milhões. A média histórica (até 2020) para este mês é de US\$ 7,4 milhões. Em `r strftime(atual, format = "%B")` de 2021 foram gerados cerca de US\$ 15,7 milhões em receitas e, em `r strftime(atual, format = "%B")` de 2022, um total de US$ 11,6 milhões.

As exportações se concentraram principalmente na variedade Palmer. Em relação aos destinos, em 2023, 47,324\% das mangas foram enviadas para a Holanda, 32,59\% para a Espanha, 7,94\% para Portugal e 4,88\% para o Reino Unido. Sobre às vias de exportação, 85,63\% foi marítima, 9,80\% aérea e 4,57\% rodoviária. A maior parte das frutas teve origem em Pernambuco (53,33\%) e na Bahia (37,92\%). São Paulo (3,68\%), Rio Grande do Norte (3,07\%) e Ceará (1,19\%) também contribuíram para os resultados obtidos com as exportações de manga.

A taxa de câmbio esperada pelo mercado para o final do ano é de R\$4,98 para 1 dólar.
