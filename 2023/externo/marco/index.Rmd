...
today <- as.Date("2023-04-07") #data para o IGPI - ultima semana do preço
mes <- 3
...
Em relação aos volumes, em `r strftime(atual, format = "%B")` foram exportadas 14,4 mil toneladas (t) de mangas pelo Brasil. Este valor é superior ao que foi exportado pelo país, no mesmo mês, em 2021 (10,6 mil t), e maior do que em `r strftime(atual, format = "%B")` de 2022 (12,3 mil t). Em termos percentuais, neste mês, em 2023, as exportações foram 35,88% maiores do que em 2021 e 17,87% maiores na comparação com 2022. Além disso, o valor de `r strftime(atual, format = "%B")` supera a média histórica, que é de 9609 t.

Em termos dos valores das exportações, em `r strftime(atual, format = "%B")` foram US\$ 12,9 milhões. A média histórica (até 2020) para este mês é de US\$ 10,7 milhões. Em `r strftime(atual, format = "%B")` de 2021 foram gerados US\$ 12,1 milhões em receitas e, em `r strftime(atual, format = "%B")` de 2022, um total de US$ 11,1 milhões.

As exportações se concentraram na variedade Palmer e Tommy para a Europa. Em relação aos destinos, em 2023, 44,79\% das mangas foram enviadas para a Holanda, 35,04\% para a Espanha, 8,61\% para Portugal e 4,38\% para o Reino Unido. Sobre às vias de exportação, 85,83\% foi marítima, 13,19\% aérea e 0,98\% rodoviária. A maior parte das frutas teve origem em Pernambuco (56,9\%) e na Bahia (31,43\%). São Paulo (5,03\%), Rio Grande do Norte (4,04\%) e Ceará (1,65\%) também contribuíram para os resultados obtidos com as exportações de manga.

A taxa de câmbio esperada pelo mercado para o final do ano é de R\$5,22 para 1 dólar.
