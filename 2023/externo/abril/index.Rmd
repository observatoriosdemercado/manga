...
atual <-  as.Date("2023-04-01") #ultimo mes disponibilizado
today <- as.Date("2023-05-05") #data para o IGPI - ultima semana do preço
mes <- 4
...
Em relação aos volumes, em `r strftime(atual, format = "%B")` foram exportadas 9,3 mil toneladas (t) de mangas pelo Brasil. Este valor é cerca de metade do que foi exportado pelo país, no mesmo mês, em 2021 (18,4 mil t), e em `r strftime(atual, format = "%B")` de 2022 (17,9 mil t). Contudo, o valor de `r strftime(atual, format = "%B")` supera a média histórica, que é de 8,2 mil t, aproximadamente.

Em termos dos valores das exportações, em `r strftime(atual, format = "%B")` foram US\$ 8,9 milhões. A média histórica (até 2020) para este mês é de US\$ 9,8 milhões. Em `r strftime(atual, format = "%B")` de 2021 foram gerados cerca de US\$ 19,9 milhões em receitas e, em `r strftime(atual, format = "%B")` de 2022, um total de US$ 15,9 milhões.

As exportações se concentraram na variedade Palmer e Tommy para a Europa. Em relação aos destinos, em 2023, 45,34\% das mangas foram enviadas para a Holanda, 34,33\% para a Espanha, 8,96\% para Portugal e 4,73\% para o Reino Unido. Sobre às vias de exportação, 86,04\% foi marítima, 10,90\% aérea e 3,06\% rodoviária. A maior parte das frutas teve origem em Pernambuco (54,1\%) e na Bahia (33,97\%). São Paulo (4,77\%), Rio Grande do Norte (4,574\%) e Ceará (1,67\%) também contribuíram para os resultados obtidos com as exportações de manga.
