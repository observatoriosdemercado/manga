...
today <- as.Date("2023-03-03") #data para o IGPI - ultima semana do preço
mes <- 2
...
Em relação aos volumes, em `r strftime(atual, format = "%B")` foram exportadas 8295 toneladas (t) de mangas pelo Brasil. Este valor é superior ao que foi exportado pelo país, no mesmo mês, em 2021, mas é menor do que em `r strftime(atual, format = "%B")` de 2022. Em termos percentuais, em 2023 as exportações foram 7,56% maiores do que em 2021 e -10,06% menores na comparação com 2022. Além disso, o valor de `r strftime(atual, format = "%B")` supera a média histórica, que é de 5920 t.

Em termos dos valores da exportações, em `r strftime(atual, format = "%B")` foram US\$ 7,45 milhões. A média histórica (até 2020) para este mês é de US\$ 6,46 milhões. Em `r strftime(atual, format = "%B")` de 2021 foram gerados US\$ 7,95 milhões em receitas e, em `r strftime(atual, format = "%B")` de 2022, um total de US$ 8,46 milhões.

As exportações se concentraram na variedade Palmer e Tommy para a Europa. Em relação aos destinos, em 2023, 40,98\% das mangas foram enviadas para a Holanda, 36,79\% para a Espanha, 10,90\% para Portugal e 3,95\% para o Reino Unido. Sobre às vias de exportação, 85,83\% foi marítima, 13,19\% aérea e 0,98\% rodoviária. A maior parte das frutas teve origem em Pernambuco (60,39\%) e na Bahia (28,10\%). São Paulo (5,53\%), Rio Grande do Norte (3,09\%) e Ceará (2,36\%) também contribuíram para os resultados obtidos com as exportações de manga.

A taxa de câmbio esperada pelo mercado para o final do ano é de R\$5,23 para 1 dólar.
