...
library(lubridate)
anterior <- as.Date("2023-07-01")
atual <-  as.Date("2023-08-01") #ultimo mes disponibilizado
today <- as.Date("2023-09-03") #data para o IGPI - ultima semana do preço
mes <- 8
...
Em relação aos volumes, em `r strftime(atual, format = "%B")` foram exportadas cerca de 20,6 mil toneladas (t) de mangas pelo Brasil. Este valor é menor do que o exportado pelo país, no mesmo mês, em 2021 (21,6 mil t), mas é maior do que em `r strftime(atual, format = "%B")` de 2022 (13 mil t), além de superar a média histórica, que é de 13,2 mil t, aproximadamente. Nos oito meses do ano, foram exportadas cerca de 105,7 mil t, um aumento de 15,78% em relação ao mesmo período de 2022.

Em termos dos valores das exportações, em `r strftime(atual, format = "%B")` foram US\$ 20,5 milhões. A média histórica (até 2020) para este mês é de US\$ 14,1 milhões. Em `r strftime(atual, format = "%B")` de 2021 foram gerados cerca de US\$ 19 milhões em receitas e, em `r strftime(atual, format = "%B")` de 2022, um total de US$ 13 milhões. Assim, as receitas médias aumentaram, indicando uma valorização da manga brasileira.

As exportações se concentraram principalmente na variedade Palmer, iniciando os embarques de Tommy para os Estados Unidos. Em relação aos destinos, em 2023, 49,14\% das mangas foram enviadas para a Holanda, 29,50\% para a Espanha, 6,56\% para Portugal e 5,19\% para o Reino Unido. Sobre as vias de exportação, 86,27\% foi marítima, 8,93\% aérea e 4,80\% rodoviária. A maior parte das frutas teve origem em Pernambuco (54,24\%) e na Bahia (38,71\%). São Paulo (3,29\%), Rio Grande do Norte (2,06\%) e Ceará (1,02\%) também contribuíram para os resultados obtidos com as exportações de manga.

A taxa de câmbio esperada pelo mercado para o final do ano é de R\$4,95 para 1 dólar.
