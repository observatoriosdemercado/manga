...
library(lubridate)
anterior <- as.Date("2022-12-01")
atual <-  as.Date("2023-01-01") #ultimo mes disponibilizado
today <- as.Date("2023-02-03") #data para o IGPI - ultima semana do preço
mes <- 1
...
dates <- dados6$Data[dados6$DataReferencia=='12/2023']
cambio_esperado <- dados6$Media[dados6$DataReferencia=='12/2023']
...
Em relação aos volumes, em `r strftime(atual, format = "%B")` foram exportadas 8549 toneladas (t) de mangas pelo Brasil. Este valor é superior ao que foi exportado pelo país, no mesmo mês, em 2021 e em 2022. Em termos percentuais, em 2023 as exportações foram 29,94% maiores do que em 2022 e 11,96% superiores na comparação com 2021. Além disso, o valor de `r strftime(atual, format = "%B")` supera a média histórica, que é de 5916 t.

Em termos dos valores da exportações, em `r strftime(atual, format = "%B")` foram US\$ 7,4 milhões. A média histórica (até 2020) para este mês é de US\$ 6,64 milhões. Em janeiro de 2021 foram gerados US\$ 7,46 milhões em receitas e, em janeiro de 2022, um total de US$ 6,15 milhões, ou seja, houve uma pequena queda nos valores recebidos do exterior em relação a 2021 e um aumento de 20,36% na comparação com o mesmo período de 2022.

As exportações se concentraram na variedade Palmer. Em relação aos destinos, em 2023, 42,78\% das mangas foram enviadas para a Holanda, 32,59\% para a Espanha, 10,53\% para Portugal e 3,78\% para o Reino Unido. Sobre às vias de exportação, 86,58\% foi marítima, 12,89\% aérea e 0,53\% rodoviária. A maior parte das frutas teve origem em Pernambuco (61,57\%) e na Bahia (28,67\%). São Paulo (6,06\%) e Rio Grande do Norte (2,53\%) também contribuíram para os resultados obtidos com as exportações de manga. 
