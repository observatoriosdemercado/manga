...
atual <-  as.Date("2023-10-01") #ultimo mes disponibilizado
today <- as.Date("2023-11-03") #data para o IGPI - ultima semana do preço
mes <- 10
...
Em relação aos volumes, em `r strftime(atual, format = "%B")` foram exportadas cerca de 46,1 mil toneladas (t) de mangas pelo Brasil. Este valor é maior do que o exportado pelo país, no mesmo mês, em 2021 (42,9 mil t), do que em `r strftime(atual, format = "%B")` de 2022 (34,8 mil t), além de superar a máxima histórica (2012 a 2020), que é de 41,3 mil t, aproximadamente. Nos dez meses do ano, foram exportadas cerca de 187 mil t, um aumento de 18,33% em relação ao mesmo período de 2022.

Em termos dos valores das exportações, em `r strftime(atual, format = "%B")` foram US\$ 51,9 milhões. O máximo histórico (até 2020) para este mês é de US\$ 40,8 milhões. Em `r strftime(atual, format = "%B")` de 2021, foram gerados cerca de US\$ 37,6 milhões em receitas e, em `r strftime(atual, format = "%B")` de 2022, um total de US$ 28,8 milhões. Assim, as receitas médias aumentaram, indicando uma valorização da manga brasileira.

As exportações se concentraram principalmente na variedade Palmer para a Europa e Tommy para os Estados Unidos. Em relação aos destinos, em 2023, 43,3\% das mangas foram enviadas para a Holanda, 20,2\% para a Espanha, 17,4\% para os Estados Unidos, 5,4\% para o Reino Unido e 4,6\% para Portugal. Sobre as vias de exportação, 88,9\% foi marítima, 7,4\% aérea e 3,6\% rodoviária. A maior parte das frutas teve origem na Bahia (47,50\%) e em Pernambuco (46,10\%). São Paulo (3\%), Rio Grande do Norte (2,204\%) e Ceará (0,70\%) também contribuíram para os resultados obtidos com as exportações de manga.
...
g4 <- ggplot()+
  ...
  geom_line(data=tablea2, aes(x=Meses, y=value, colour=variable), linetype = "solid",
    ...
    scale_y_continuous(limits = c(0, 55), n.breaks = 10)+
    scale_x_date(date_breaks = "1 month",
               labels = date_format("%B"))+
