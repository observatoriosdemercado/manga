...
library(lubridate)
anterior <- as.Date("2022-06-01")
atual <-  as.Date("2022-07-01") #ultimo mes disponibilizado
today <- as.Date("2022-07-29") #data para o IGPI - ultima semana do preço
mes <- 7
...
O objetivo desta análise foi verificar o comportamento das exportações de manga do Brasil nos primeiros sete meses de 2022. Os dados utilizados são mensais e disponibilizados pelo **Comexstat (MDIC)** para o período de *janeiro de 2012 a `r strftime(atual, format = "%B de %Y")`*.

Em relação aos volumes, em `r strftime(atual, format = "%B")` foram de 11.933 toneladas (t). A média histórica (até 2019, antes da pandemia) para este mês é de 7.229 t. Em `r strftime(atual, format = "%B")` de 2020 foram exportadas 11.945 mil t e, em `r strftime(atual, format = "%B")` de 2021, foram 11.129 t, ou seja, houve uma relativa estabilidade na comparação com o mesmo período dos dois anos anteriores. Se for feita uma conversão para contêineres, o volume exportado em `r strftime(atual, format = "%B de %Y")` é de cerca de `r round((tail(dados2[,4],1))/22216,0)` contêineres.  

Em termos dos valores da exportações, em `r strftime(atual, format = "%B")` foram de 11,198 milhões de dólares (US\$). A média histórica (até 2019, antes da pandemia) para este mês é de US\$ 8,51 milhões. Em `r strftime(atual, format = "%B")` de 2020 foram gerados US\$ 12,815 milhões em receitas e, em `r strftime(atual, format = "%B")` de 2021, um total de US\$ 9,477 milhões, ou seja, houve um aumento nos valores recebidos do exterior de 18,16% em relação a 2021 e uma queda de -12,62% na comparação com o mesmo período de 2020.   

As exportações se concentraram na variedade Palmer e Tommy para a Europa. O preço médio ao produtor no Vale do São Francisco ficou em cerca de R\$ 2,65, ao longo do mes, para a Palmer e R\$ 2,58 a R\$3,20 para o Tommy. 

Em relação aos destinos, nos primeiros sete meses 46,74\% das mangas foram enviadas para a Holanda, 30,15\% para a Espanha, 8,31\% para Portugal e 4,86\% para o Reino Unido. Em relação às vias de exportação, 84,09\% foi marítima, 11,14\% aérea e 4,77\% rodoviária. A maior parte das frutas teve origem em Pernambuco (51,67\%) e Bahia (38,00\%). São Paulo (4,86\%), Rio Grande do Norte (2,48\%) e Ceará (1,43\%) também contribuíram para o resultado obtido com as exportações de manga. 
