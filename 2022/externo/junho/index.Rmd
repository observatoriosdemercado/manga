...
O objetivo desta análise foi verificar o comportamento das exportações de manga do Brasil nos primeiros seis meses de 2022. Os dados utilizados são mensais e disponibilizados pelo **Comexstat (MDIC)** para o período de *janeiro de 2012 a `r strftime(atual, format = "%B de %Y")`*.

Em relação aos volumes, em `r strftime(atual, format = "%B")` foram de 12.095 toneladas (t). A média histórica (até 2019, antes da pandemia) para este mês é de 5.763 t. Em `r strftime(atual, format = "%B")` de 2020 foram exportadas 9.850 mil t e, em `r strftime(atual, format = "%B")` de 2021, foram 17.247 t, ou seja, houve um aumento de 22,79% em relação a 2020, e uma redução de -29,90% na comparação com o mesmo período do ano passado. Se for feita uma conversão para contêineres, o volume exportado em `r strftime(atual, format = "%B de %Y")` é de cerca de `r round((tail(dados2[,4],1))/22216,0)` contêineres.  

Em termos dos valores da exportações, em `r strftime(atual, format = "%B")` foram de 11,18 milhões de dólares (US\$). A média histórica (até 2019, antes da pandemia) para este mês é de US\$ 6,99 milhões. Em `r strftime(atual, format = "%B")` de 2020 foram gerados US\$ 10,805 milhões em receitas e, em `r strftime(atual, format = "%B")` de 2021, um total de US\$ 15,677 milhões, ou seja, houve um aumento nos valores recebidos do exterior de 3,51% em relação a 2020 e uma queda de -28,66% na comparação com o mesmo período de 2021.   

As exportações se concentraram na variedade Palmer e Tommy para a Europa. O preço médio ao produtor no Vale do São Francisco variou entre R\$ 2,20 e R\$2,66, ao longo do mes, para a Palmer e R\$ 2,65 a R\$3,00 para o Tommy. 

Em relação aos destinos, nos primeiros seis meses 45,89\% foi enviada para a Holanda, 30,74\% para a Espanha, 8,67\% para Portugal e 3,98\% para o Reino Unido. Em relação às vias de exportação, 83,49\% foi marítima, 11,52\% área e 4,99\% rodoviária. A maior parte das frutas teve origem em Pernambuco (50,40\%) e Bahia (38,09\%). São Paulo (5,10\%), Rio Grande do Norte (2,92\%) e Ceará (1,68\%) também contribuíram para o resultado obtido com as exportações de manga. 
