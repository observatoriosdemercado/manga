...
library(lubridate)
anterior <- as.Date("2022-10-01")
atual <-  as.Date("2022-11-01") #ultimo mes disponibilizado
today <- as.Date("2022-12-02") #data para o IGPI - ultima semana do preço
mes <- 11
...
Em relação aos volumes, em `r strftime(atual, format = "%B")` foram exportadas 31.421 toneladas (t). A média histórica (até 2019, antes da pandemia) para este mês é de 27.069 t; em `r strftime(atual, format = "%B")` de 2020 foram exportadas 46.884 t e, em `r strftime(atual, format = "%B")` de 2021, foram 47.495 t, ou seja, houve uma queda na comparação com o mesmo período dos dois anos anteriores. Se for feita uma conversão para contêineres, o volume exportado em `r strftime(atual, format = "%B de %Y")` é de cerca de `r round((tail(dados2[,4],1))/22216,0)` contêineres.  

Em termos dos valores da exportações, em `r strftime(atual, format = "%B")` foram de 26,3 milhões de dólares (US\$). A média histórica (até 2019, antes da pandemia) para este mês é de US\$ 31,1 milhões. Em `r strftime(atual, format = "%B")` de 2020 foram gerados US\$ 47,96 milhões em receitas e, em `r strftime(atual, format = "%B")` de 2021, um total de US\$ 40,1 milhões, ou seja, houve uma queda nos valores recebidos do exterior de -45,20% em relação a 2020 e de -34,54% na comparação com o mesmo período de 2021.   

As exportações se concentraram nas variedades Palmer, Tommy e Kent. O preço médio ao produtor no Vale do São Francisco variou entre R\$ 1,60 e R\$ 1,36, ao longo do mês, para a Palmer, de R\$1,36 a R\$0,98 para a Tommy e entre R\$2,25 e R\$1,98 para a Kent.

Em relação aos destinos, nos onze meses de 2022, 43,18\% das mangas foram enviadas para a Holanda, 19,11\% para os Estados Unidos, 16,43\% para a Espanha e 7,03\% para o Reino Unido. Em relação às vias de exportação, 88,45\% foi marítima, 7,59\% aérea e 3,96\% rodoviária. A maior parte das frutas teve origem em Pernambuco (46,51\%) e Bahia (44,99\%). São Paulo (3,41\%), Rio Grande do Norte (3,17\%) e Ceará (0,88\%) também contribuíram para os resultados obtidos com as exportações de manga. 

A taxa de câmbio esperada pelo mercado para o final do ano é de R$5,26 para 1 dólar. 
