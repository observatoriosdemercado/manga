...
atual <-  as.Date("2022-09-01") #ultimo mes disponibilizado
today <- as.Date("2022-10-07") #data para o IGPI - ultima semana do preço
mes <- 9
...
Em relação aos volumes, em `r strftime(atual, format = "%B")` foram exportadas 32.243 toneladas (t). A média histórica (até 2019, antes da pandemia) para este mês é de 23.021 t; em `r strftime(atual, format = "%B")` de 2020 foram exportadas 35.302 mil t e, em `r strftime(atual, format = "%B")` de 2021, foram 42.151 t, ou seja, houve uma queda na comparação com o mesmo período dos dois anos anteriores. Se for feita uma conversão para contêineres, o volume exportado em `r strftime(atual, format = "%B de %Y")` é de cerca de `r round((tail(dados2[,4],1))/22216,0)` contêineres.  

Em termos dos valores da exportações, em `r strftime(atual, format = "%B")` foram de 27,3 milhões de dólares (US\$). A média histórica (até 2019, antes da pandemia) para este mês é de US\$ 24,7 milhões. Em `r strftime(atual, format = "%B")` de 2020 foram gerados US\$ 36,7 milhões em receitas e, em `r strftime(atual, format = "%B")` de 2021, um total de US\$ 37,5 milhões, ou seja, houve uma queda nos valores recebidos do exterior de -25,77% em relação a 2020 e de -27,27% na comparação com o mesmo período de 2020.   

As exportações se concentraram nas variedades Palmer e Tommy. O preço médio ao produtor no Vale do São Francisco variou entre R\$ 1,73 e R\$ 1,50, ao longo do mês, para a Palmer e de R\$1,98 a R\$1,60 para a Tommy. 

Em relação aos destinos, nos nove meses de 2022, 44,34\% das mangas foram enviadas para a Holanda, 22,10\% para a Espanha, 12,21\% para os Estados Unidos e 6,2\% para Portugal. Em relação às vias de exportação, 86,32\% foi marítima, 9,05\% aérea e 4,64\% rodoviária. A maior parte das frutas teve origem em Pernambuco (49,06\%) e Bahia (42,42\%). São Paulo (3,94\%), Rio Grande do Norte (2,41\%) e Ceará (0,90\%) também contribuíram para os resultados obtidos com as exportações de manga. 

A taxa de câmbio esperada pelo mercado para o final do ano é de R$5,20 para 1 dólar. 
