...
today <- as.Date("2023-01-13") #data para o IGPI - ultima semana do preço
mes <- 12
...
# O comportamento das exportações de manga do Brasil: Janeiro a `r strftime(atual, format = "%B de %Y")`.
...
Em relação aos volumes, em `r strftime(atual, format = "%Y")` foram exportadas 231.364 toneladas (t) de mangas pelo Brasil. Este valor é inferior ao que foi exportado pelo país em 2020 e em 2021. Em termos percentuais, em 2022 as exportações foram 15,11% menores do que em 2021 e 4,88% inferiores na comparação com 2020. O valor de 2022 vai superar apenas o ano de 2019, quando o Brasil exportou cerca de 216 mil t. 

Em termos dos valores da exportações, os valores ainda devem sofrer ajustes de correções no próximo mês, mas é evidente que os valores arrecadados serão bastante inferiores aos anos anteriores, só superando o ano de 2018. De acordo com o governo brasileiro, as receitas de exportação da manga foram 205,7 milhões de dólares, um valor 17,32% menor do que o ano de 2021, 16,88% menor do que em 2020 e 7,28% mais baixo do que em 2019. Com base nos percentuais, pode-se inferir que o Brasil recebeu, na média do ano, preços mais baixos do que em outras safras, ou seja, precisou enviar muito mais fruta para poder receber um igual montante de recursos. 

As exportações se concentraram nas variedades Palmer, Tommy, Keitt e Kent. Em relação aos destinos, em 2022, 46,79\% das mangas foram enviadas para a Holanda, 17,42\% para os Estados Unidos, 15,79\% para a Espanha e 7,24\% para o Reino Unido. Em relação às vias de exportação, 89,70\% foi marítima, 6,87\% aérea e 3,43\% rodoviária. A maior parte das frutas teve origem na Bahia (46,07\%) e em Pernambuco (45,05\%). Rio Grande do Norte (3,54\%), São Paulo (3,45\%)  e Ceará (0,98\%) também contribuíram para os resultados obtidos com as exportações de manga. 
