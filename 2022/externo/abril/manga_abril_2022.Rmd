...
O objetivo desta análise foi verificar o comportamento das exportações de manga do Brasil no primeiro quadrimestre de 2022. Os dados utilizados são mensais e disponibilizados pelo **Comexstat (MDIC)** para o período de *janeiro de 2012 a `r strftime(atual, format = "%B de %Y")`*.

Em relação aos volumes, em `r strftime(atual, format = "%B")` foram de 17,914 mil toneladas (t). A média histórica (até 2019, antes da pandemia) para este mês é de 7,9 mil t. Em `r strftime(atual, format = "%B")` de 2020 foram exportadas 10,660 mil t e, em `r strftime(atual, format = "%B")` de 2021, foram 18,4 mil t, ou seja, houve um aumento de 68% em relação a 2020, mas uma queda de -2,64% na comparação com o mesmo período do ano passado. Se for feita uma conversão para contêineres, o volume exportado em `r strftime(atual, format = "%B de %Y")` é de cerca de `r round((tail(dados2[,4],1))/22216,0)` contêineres.  

Em termos dos valores da exportações, em `r strftime(atual, format = "%B")` foram de 16,375 milhões de dólares (US\$). A média histórica (até 2019, antes da pandemia) para este mês é de US\$ 9,8 milhões. Em `r strftime(atual, format = "%B")` de 2020 foram gerados US\$ 9,579 milhões em receitas e, em `r strftime(atual, format = "%B")` de 2021, um total de US\$ 19,883 milhões, ou seja, houve um aumento nos valores recebidos do exterior de 70,95% em relação a 2020 e uma queda de -17,64% na comparação com o mesmo período de 2021.   
...
Obs: Dados de 2022 até o mês de abril.
