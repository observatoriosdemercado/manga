...
library(lubridate)
anterior <- as.Date("2022-07-01")
atual <-  as.Date("2022-08-01") #ultimo mes disponibilizado
today <- as.Date("2022-09-02") #data para o IGPI - ultima semana do preço
mes <- 8
...
Em relação aos volumes, em `r strftime(atual, format = "%B")` foram exportadas 13.040 toneladas (t). A média histórica (até 2019, antes da pandemia) para este mês é de 12.194 t; em `r strftime(atual, format = "%B")` de 2020 foram exportadas 20.893 mil t e, em `r strftime(atual, format = "%B")` de 2021, foram 21.574 t, ou seja, houve uma forte queda na comparação com o mesmo período dos dois anos anteriores. Se for feita uma conversão para contêineres, o volume exportado em `r strftime(atual, format = "%B de %Y")` é de cerca de `r round((tail(dados2[,4],1))/22216,0)` contêineres.  

Em termos dos valores da exportações, em `r strftime(atual, format = "%B")` foram de 12,597 milhões de dólares (US\$). A média histórica (até 2019, antes da pandemia) para este mês é de US\$ 13,05 milhões. Em `r strftime(atual, format = "%B")` de 2020 foram gerados US\$ 22,137 milhões em receitas e, em `r strftime(atual, format = "%B")` de 2021, um total de US\$ 19,053 milhões, ou seja, houve uma queda nos valores recebidos do exterior de -43,10% em relação a 2020 e de -33,88% na comparação com o mesmo período de 2020.   

As exportações se concentraram na variedade Palmer. O preço médio ao produtor no Vale do São Francisco variou entre R\$ 2,28 e R\$ 1,74, ao longo do mês, para esta variedade. 

Em relação aos destinos, nos oito meses de 2022, 46,63\% das mangas foram enviadas para a Holanda, 28,38\% para a Espanha, 8\% para Portugal e 5,36\% para o Reino Unido. Em relação às vias de exportação, 83,79\% foi marítima, 11,11\% aérea e 5,11\% rodoviária. A maior parte das frutas teve origem em Pernambuco (52,43\%) e Bahia (37,81\%). São Paulo (4,66\%), Rio Grande do Norte (2,39\%) e Ceará (1,22\%) também contribuíram para os resultados obtidos com as exportações de manga. 
