...
Os dados utilizados nesta análise são mensais e disponibilizados pela *CONAB/PROHORT*. São para a oferta de manga comercializados nos CEASAS do Brasil com origem em todos os estados e comparados com o volume de origem Pernambuco e Bahia. O período disponibilizado é de **janeiro de 2017 à fevereiro de 2022**;

Em relação aos *volumes (total e com origem Pernambuco e Bahia)*, considerando os meses de janeiro e fevereiro de 2022, 47,93\% de toda a manga comercializada nos CEASAS do Brasil, a origem foi Pernambuco e Bahia, um total de 23,6 mil toneladas dentre as 49,2 mil toneladas que passaram pelos CEASAS;  

A informação da variação do volume, comparando `r strftime(atual, format = "%B de %Y")` com 2021, mostra uma estabilidade (0,07\%), cenário bem diferente do que ocorreu em 2021 com 2020, quando o volume foi 23,16% mais elevado. Assim, possivelmente como efeito das chuvas que ocorreram em Pernambuco e na Bahia, os volumes (até o momento) não conseguiram aumentar na mesma velocidade que cresceram no ano anterior;

A **Tendência** entre 2017 e 2022 não mostra crescimento significativo do volume. A **Sazonalidade** mostra que a partir de março deve existir um crescimento da oferta até maio e, depois de uma estabilidade até agosto, se tem um novo crescimento até outubro, quando atinge o pico de oferta;

A comparação da média, mínimo valor e máximo valor (até 2019) com os valores de 2020, 2021 e 2022 mostra que nos últimos anos os volumes foram maiores do que o que se teve de máximo até 2019, ou seja, um grande volume de manga no mercado interno. 
...
ggplotly(g1) %>%
  layout(legend = list(
                      ...
                      title=''))
```
\footnotesize Obs: Dados de 2022 apenas até o mês de `r strftime(atual, format = "%B")`.
...
Obs: Dados de 2022 apenas até o mês de `r strftime(atual, format = "%B")`.
