...
Em `r strftime(atual, format = "%B de %Y")`, o Estoque de empregos na cultura da manga é de 12.865 pessoas. No mesmo mês do ano passado, este valor era de 11.877, ou seja, existe um aumento de 8,32% no número de pessoas ocupadas na mangicultura.

O saldo de empregos na mangicultura em `r strftime(atual, format = "%B de %Y")` no Vale do São Francisco foi negativo, de -588 pessoas, ou seja, o total de contratações (538) foi menor menor do que o total de demissões (1.126).   Em relação ao mesmo mês do ano passado, se tem uma melhora no saldo total (-834 em 2024 e -588 em 2025), com mais pessoas mantendo o emprego. 

Dos contratados no mês, 358 são do sexo masculino (66,54%) e 180 do sexo feminino (33,46%). Em relação à idade, 52,23% se concentra abaixo dos 30 anos: 80 pessoas até 21 anos, 65 até 24 anos, 68 ate 27 anos e 68 até 30 anos. 

Sobre o nível de escolaridade, a maioria das admissões é de pessoas com ensino médio completo (195). Do total de contratados, 36,24% possuem este nível de escolaridade. 

Com relação aos salários pagos, 85,5% dos contratados foram pelo valor de R\$ 1.555,00 (valor definido no CCT 2025), mais alto que o salário mínimo nacional. O maior salário foi R\$ 7.567,00. 
...
#Entrando dados no R
dados1 <- read.csv2('dezembro_2025.csv', header=T, sep=";")
...
#Entrando dados no R
dados2 <- read.csv2('estoque_dezembro25.csv', header=T, sep=";")
...
estoque_wide <- estoquem %>%
  ...
  complete(ano = c(2024L, 2025L), 
           mes = factor(niv_meses, levels = niv_meses, ordered = TRUE)
           ) %>%  # <— usa inteiros (L)
  group_by(ano, mes) %>%
  summarise(
    manga_total = if (all(is.na(manga_total))) NA_real_ else sum(manga_total, na.rm = TRUE),
    .groups = "drop"
  ) %>%
