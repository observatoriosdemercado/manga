...
today <- as.Date("2025-03-07") #data para o IGPI - ultima semana do preço
mes <- 2
```
...
Em relação aos volumes, em `r strftime(atual, format = "%B")` foram exportadas cerca de 10,4 mil toneladas (t) de mangas pelo Brasil. Este valor é maior do que a média histórica (7,1 mil t), mais alto do que os volumes de 2023 (8,3 mil t), mas menor do que o volume de 2024 (13,5 mil t). 

Em termos dos valores das exportações, em `r strftime(atual, format = "%B")` foram, aproximadamente, US\$ 9,8 milhões. Este valor supera a média histórica (US\$ 8,1 milhões), o valor de 2023 (US\$ 7,7 milhões), mas é menor do que o valor de 2024 US\$ 22,9 milhões.

As exportações se concentraram, principalmente, na variedade Palmer. Em relação aos principais destinos, no ano, 48,5% das mangas foram enviadas para a Holanda, 31,56% para a Espanha e 10,62% para Portugal. 

A principal via de envio foi a marítima (84,56%), com 76,4% do total despachado pela alfândega de Fortaleza e 7,95% pelo Aeroporto de Guarulhos-SP.

A maior parte das frutas teve origem em Pernambuco (62,50%) e na Bahia (23,44%). Os estados de São Paulo (5,58%) e Rio Grande do Norte (4,74%), também contribuíram para os resultados obtidos com as exportações de manga.
...
dados21$URF <- factor(dados21$URF, levels = c("ALF-Fortaleza", "Aeroporto de GRU","IRF Natal", "Aeroporto de VCP", "ALF-Belo Horizonte", "ALF-Dionísio Cerqueira", "ALF-Salvador",  "Outros"))
