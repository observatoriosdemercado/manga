...
library(lubridate)
anterior <- as.Date("2024-12-01")
atual <-  as.Date("2025-01-01") #ultimo mes disponibilizado
today <- as.Date("2025-02-07") #data para o IGPI - ultima semana do preço
mes <- 1
```
...
Em relação aos volumes, em `r strftime(atual, format = "%B")` foram exportadas cerca de 11,1 mil toneladas (t) de mangas pelo Brasil. Este valor é maior do que a média histórica (6,7 mil t), mais alto do que os volumes de 2023 (8,5 mil t) e o volume de 2024 (10,9 mil t). 

Em termos dos valores das exportações, em `r strftime(atual, format = "%B")` foram, aproximadamente, US\$ 9,5 milhões. Este valor supera a média histórica (US\$ 7,5 milhões), o valor de 2023 (US\$ 7,3 milhões), mas é menor do que o valor de 2024 US\$ 16,4 milhões.

As exportações se concentraram, principalmente, na variedade Palmer. Em relação aos principais destinos, no ano, 51,37% das mangas foram enviadas para a Holanda, 28,84% para a Espanha e 10,44% para os Estados Unidos. 

A principal via de envio foi a marítima (84,67%), com 80,47% do total despachado pela alfândega de Fortaleza e 8,21% pelo Aeroporto de Guarulhos-SP.

A maior parte das frutas teve origem em Pernambuco (64,51%) e na Bahia (21,8%). Os estados de São Paulo (5,28%) e Rio Grande do Norte (4,21%), também contribuíram para os resultados obtidos com as exportações de manga.
...
dados21$URF <- factor(dados21$URF, levels = c("ALF-Fortaleza", "Aeroporto de GRU","IRF Natal", "Aeroporto de VCP", "ALF-Salvador", "ALF-Foz",  "Outros"))
