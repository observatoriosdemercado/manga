...
library(lubridate)
anterior <- as.Date("2025-02-01")
atual <-  as.Date("2025-03-01") #ultimo mes disponibilizado
today <- as.Date("2025-04-11") #data para o IGPI - ultima semana do preço
mes <- 3
```
...
Em relação aos volumes, em `r strftime(atual, format = "%B")` foram exportadas cerca de 18 mil toneladas (t) de mangas pelo Brasil. Este valor é maior do que a média histórica (11,1 mil t), mais alto do que os volumes de 2023 (14,4 mil t), mas menor do que o volume de 2024 (21 mil t). No ano, foram exportadas 39,6 mil t, queda de 13% em relação à 2024.

Em termos dos valores das exportações, em `r strftime(atual, format = "%B")` foram, aproximadamente, US\$ 19 milhões. Este valor supera a média histórica (US\$ 13,2 milhões), o valor de 2023 (US\$ 13,6 milhões), mas é menor do que o valor de 2024, US\$ 36,1 milhões. No ano, as receitas chegam a US\$ 38,5 milhões, redução de 46% em relação à 2024.

As exportações se concentraram, principalmente, nas variedades Palmer e Tommy. Em relação aos principais destinos, no ano, 49,84% das mangas foram enviadas para a Holanda, 28,77% para a Espanha e 9,2% para Portugal. 

A principal via de envio foi a marítima (85,94%), com 80,4% do total despachado pela alfândega de Fortaleza e 6,97% pelo Aeroporto de Guarulhos-SP.

A maior parte das frutas teve origem em Pernambuco (58,96%) e na Bahia (26,06%). Os estados de São Paulo (6,17%) e Rio Grande do Norte (5,30%), também contribuíram para os resultados obtidos com as exportações de manga.
...
dados21$URF <- factor(dados21$URF, levels = c("ALF-Fortaleza", "Aeroporto de GRU","IRF Natal",  "ALF-Salvador", "Aeroporto de VCP", "ALF-Foz do Iguaçu", "ALF-Belo Horizonte", "Outros"))
