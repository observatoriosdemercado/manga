...
library(lubridate)
anterior <- as.Date("2025-03-01")
atual <-  as.Date("2025-04-01") #ultimo mes disponibilizado
today <- as.Date("2025-05-09") #data para o IGPI - ultima semana do preço
mes <- 4
```
...
Em relação aos volumes, em `r strftime(atual, format = "%B")` foram exportadas cerca de 24,1 mil toneladas (t) de mangas pelo Brasil. Este valor é maior do que a média histórica (10,4 mil t), mais alto do que os volumes de 2023 (9,3 mil t) e do que o volume de 2024 (16,3 mil t). No ano, foram exportadas 63,7 mil t, aumento de 3,1% em relação à 2024.

Em termos dos valores das exportações, em `r strftime(atual, format = "%B")` foram, aproximadamente, US\$ 28,4 milhões. Este valor supera a média histórica (US\$ 12,3 milhões), o valor de 2023 (US\$ 9,7 milhões) e o valor de 2024, US\$ 26 milhões. No ano, as receitas chegam a US\$ 66,9 milhões, redução de -34% em relação à 2024.

As exportações se concentraram, principalmente, nas variedades Palmer e Tommy. Em relação aos principais destinos, no ano, 52,36% das mangas foram enviadas para a Holanda, 27,41% para a Espanha e 8,02% para Portugal. 

A principal via de envio foi a marítima (86,83%), com 82,8% do total despachado pela alfândega de Fortaleza e 6% pelo Aeroporto de Guarulhos-SP.

A maior parte das frutas teve origem em Pernambuco (57,07%) e na Bahia (28,96%). Os estados de São Paulo (5,93%) e Rio Grande do Norte (4,88%), também contribuíram para os resultados obtidos com as exportações de manga.

A taxa de câmbio esperada pelo mercado para o final do ano de 2025 é de R$5,86 para 1 dólar.
...
dados21$URF <- factor(dados21$URF, levels = c("ALF-Fortaleza", "Aeroporto de GRU","IRF Natal",  "ALF-Dionísio Cerqueira", "ALF-Salvador", "Aeroporto de VCP", "ALF-Foz do Iguaçu", "ALF-Belo Horizonte", "Outros"))
