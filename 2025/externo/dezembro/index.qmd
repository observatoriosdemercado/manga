...
library(lubridate)
anterior <- as.Date("2025-11-01")
atual <-  as.Date("2025-12-01") #ultimo mes disponibilizado
today <- as.Date("2025-12-26") #data para o IGPI - ultima semana do preço
mes <- 12
```
...
Em relação aos volumes, em `r strftime(atual, format = "%B")` foram exportadas cerca de 30,5 mil toneladas (t) de mangas pelo Brasil. Este valor é maior do que a média histórica (21,7 mil t). No ano, foram exportadas cerca de 291 mil t, volume aproximadamente 13,5% mais elevado do que no mesmo período de 2024. Em relação ao mesmo mês do ano passado, se tem um aumento de 12,59%, um recorde histórico.

Por outro lado, as receitas não foram as maiores da série histórica, indicando preços menores aos produtores/exportadores, que enviaram mais frutas e receberam menores divisas. Em 2025, as receitas foram de US\$ 335,1 milhões, -4,3% na comparação com 2024, onde as receitas superaram os US\$ 350 milhões.

As exportações se concentraram nas variedades Palmer, Kent e Keitt para a Europa e Tommy Atkins para os Estados Unidos. Em relação aos principais destinos, no ano, 49,47% das mangas foram enviadas para a Holanda, 15,22% para a Espanha e 12,91% para os Estados Unidos.

A principal via de envio foi a marítima (88,31%), com 79,61% do total despachado pela alfândega de Fortaleza/CE e 5,77% pela inspetoria da receita federal de Natal/RN.

A maior parte das frutas teve origem em Pernambuco (51,49%) e na Bahia (40,82%). Os estados de São Paulo (3,5%) e Rio Grande do Norte (2,58%), também contribuíram para os resultados obtidos com as exportações de manga.
...
# Exportações de mangas do Brasil para os Estados Unidos: 2024-2025
...
df <- read.csv2('USA_24_25.csv', header=T, sep=";", dec = ".")

# 2) Renomear colunas (se necessário)
colnames(df) <- c("mes", "Toneladas_2024", "Toneladas_2025")

# 3) Converter para mil toneladas
df <- df %>%
  mutate(
    `Toneladas_2024_mil` = Toneladas_2024 / 1000,
    `Toneladas_2025_mil` = Toneladas_2025 / 1000
  )

# 4) Ordem dos meses
ordem_meses <- c("janeiro", "fevereiro", "marco", "abril", "maio", "junho",
                 "julho", "agosto", "setembro", "outubro", "novembro", "dezembro")

df$mes <- factor(tolower(df$mes), levels = ordem_meses, ordered = TRUE)

# 5) Transformar em formato longo
df_long <- df %>%
  pivot_longer(cols = c("Toneladas_2024_mil", "Toneladas_2025_mil"),
               names_to = "Ano", values_to = "valor_mil_t") %>%
  mutate(Ano = ifelse(grepl("2024", Ano), "2024", "2025"))

# 6) Criar rótulos
df_long <- df_long %>%
  mutate(label = sprintf("%.1f", valor_mil_t))

# 7) Gráfico de barras

g9a <- ggplot(df_long, aes(x = mes, y = valor_mil_t, fill = Ano)) +
  ...
  geom_text(aes(label = label),
    ...
    hjust=0.8, 
    vjust=-1.4)+
  scale_fill_manual(values = c("2024" = "#B22222",  # vermelho (EUA)
                               "2025" = "#1E3A8A")) + # azul (EUA)
  labs(title = "",
       x = "Meses",
       y = "Volume exportado para os EUA (mil ton)",
       fill = "Ano") +
  theme_minimal()+
  theme(axis.text.x=element_text(hjust=0.5, size=11, margin = margin(b=10)),
        axis.text.y=element_text(hjust=0.5, size=14, margin = margin(l=20)),
...
Fonte: COMEXSTAT (2025). Dados reprocessados pelo Observatório de Mercado de Manga da Embrapa
...
Fonte: CEPEA (2025). Dados reprocessados pelo Observatório de Mercado de Manga da Embrapa
...
dados21$URF <- factor(dados21$URF, levels = c("ALF-Fortaleza", "IRF Natal", "Aeroporto de GRU", "ALF-Salvador", "ALF-Foz do Iguaçu" , "ALF-Dionísio Cerqueira", "Aeroporto de VCP", "Aeroporto do Rio de Janeiro", "Outros"))
